...
#define DATAFILE "transactions.csv.gz"  // Note there are 142,221,150 lines
#define WITH_ZLIB
...
void initialize_vectors() {
    ...
    for (long a=0; a<CUSTS*D; a++) customer_vecs[a] = qrand_normal() / D;
    for (long a=0; a<PRODS*D; a++) product_vecs[a] = qrand_normal() / D;
    return;
...
void run(gzFile *fp) {
    ...
    while (!gzeof(fp)) {
        // get the next line from the gz file
        gzgets(fp, dump, MAX_STRING);
        linenum++;
        sscanf(dump, "%ld,%*d,%*d,%*d,%ld,%ld,%*12[^,],%*d,%*12[^,],%ld,%*s", &id, &company, &brand, &quantity);
        ...
        if (linenum%10000000 == 0) {
            FILE *fc = fopen("cust_vecs2","w");
            print_customers(fc);
            fclose(fc);
            FILE *fp = fopen("prod_vecs2","w");
            print_products(fp);
            fclose(fp);
        }
...
int main(int argc, char *argv[]) {
    initialize();
    initialize_vectors();
    gzFile *datfile = gzopen(DATAFILE,"r");
